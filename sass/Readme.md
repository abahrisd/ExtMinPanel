...
folder are mapped to the application's root namespace, 'ExtMinPanel'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "ExtMinPanel"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'ExtMinPanel' in the same way
as `"ExtMinPanel/sass/src"`.
